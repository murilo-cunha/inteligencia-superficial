...
"Gradient descent" é um algoritmo de otimização. Ele é usado pra procurar mínimos de uma função de modo iterativo, através dando passos em direção ao ponto "mais baixo" da função.
...
Como que nós podemos medir o quão bom o nosso modelo de machine learning é? Como que nós comparamos dois modelos? Acontece que existe uma função que mede exatamente isso, uma função que mede o *erro* do nosso modelo. Essa **função aceita os mesmos parâmetros do modelo**, e retorna o erro do modelo.

Por exemplo, se eu quero acertar um alvo no chão com a minha raquete, uma batida muito forte prejudica meu jogo. Mas também se eu bato muito fraco o vento empurra a bolinha. Se eu conseguisse formular uma função *matemática* que pega a força que eu bati na bola e retorna a distância da bola até o alvo, esse seria a função do erro, e eu seria o modelo.[^1]

{% include info.html text="Nós temos saber a derivada da nossa função de erro." %}
...
Essa pergunta não é nova. Desde o colegial nós aprendemos a achar mínimos e máximos de uma função. Mas e se nós tivermos várias variáveis? Cálculo multivariável, resolvendo um sistema de equações com as derivadas parciais.
...
{% include info.html text="Quando resolvemos numericamente e fazemos essas aproximações, nós aproximamos os valores mínimos da função, mas talvez (e provavelmente) nunca chegamos nesse valor mínimo." %}

* $$f \rightarrow$$ nossa função que indica o erro do nosso modelo
* $$p \rightarrow$$ o parâmetro que essa função toma
* $$e \rightarrow$$ o erro do nosso modelo para um grupo de parâmetros
...
É importante perceber que uma função de erro é dificilmente assim. Em casos mais práticos a função é bem mais bagunçada e têm vários máximos e mínimos. Também vale ressaltar que esses métodos para achar o mínimo estão à procura do [mínimo local][max_min], e não global.

#### Derivadas

Lembrando que as [derivadas][derivadas] são o [coeficiente angular][coef_ang] da linha tangente a uma curva, se a gente "cai" num ponto aleatório da nossa função de erro, a gente percebe que:
...
Ou seja, a derivada já nos indica pra que lado devemos caminhar. Mais tecnicamente, ela indica se o mínimo da nossa função está num valor maior ou menor do que o nosso $x$ atual!
...
* Ajuste o seu chute dando um passo em proporcional à derivada
* Repita o processo até que esteja satisfeito

<!-- <video width="100%" height="auto" loop autoplay controls>
  <source src="{{ site.baseurl }}/assets/manim/....mp4" type="video/mp4">
  O seu browser não suporta a visualização de vídeos.
</video> -->
...
Mas os mesmos cuidados aplicam aqui. Essa constante não pode ser muito larga, nem muito pequena.
...
Ah, e a nossa linha tangente vira o plano tangente a curva!
...
Eu sei que se você não está 100% confortável com álgebra linear isso pode assustar um pouco. Mas o que essa equação ta descrevendo são várias equações iguais à que apresentamos acima. Uma para cada dimensão do nosso problema. Ou seja:
...
* $$\alpha \rightarrow$$ uma constante que multiplicamos pela derivada, controlando o passo - a taxa de aprendizado
* $$\frac{\partial}{\partial p_{t-1}^{(d)}} f (p_{t-1}^{(d)})) \rightarrow$$ a derivadas parcial da nossa função do erro para a dimensão $d$, em relação ao parâmetro $p_{t-1}$ da dimensão $d$

$$p_{t}^{(d)} \leftarrow p_{t-1}^{(d)} - \alpha \frac{\partial}{\partial p_{t-1}^{(d)}} f (p_{t-1}^{(d)}) $$

Eu sei que ainda é muita letrinha, e que pode ser confuso. Respira fundo e vai com calma. Talvez quando falarmos de regressão linear isso também fique mais claro.
...
No começo desse post eu dei um exemplo: se eu estiver jogando tênis, tentando acertar um alvo no chão, e estou tentando ajustar a força ideal para chegar o mais próximo do alvo possível. Uma função de erro poderia ser a soma de todas as distâncias do centro do alvo até onde eu acertei a bolinha, seja 100 ou 1000.

Quando seguimos os etapas desse algorítmo, cada passo é dado depois de computar a derivada. E a derivada leva em consideração todos as nossas tentatívas (exemplos)[^3]. Ou seja, reduzimos o erro depois de  "visitarmos" essas experiências passadas. Do mesmo jeito que eu, quanto mais vezes eu tento, vou melhorando (reduzindo meu erro).

Tudo depende de como definimos "aprender". Se eu definir como "a melhora de performance com nossas experiências", não é tão incoerente falar que a máquina aprende.
...
Não vamos falar de *stochastic gradient descent* ou *online learning* nesse post. Aliás, existem muitas variações desses algoritmos de otimização. Na prática, ninguém acaba usando *gradient descent* por haver opções melhores, mas a idéia central de todos os algoritmos é a mesma.

## Outras Implementações

Existem maneiras de melhorar a performance desses algoritmos. Por exemplo, nós poderíamos reduzir o *learning rate* a cada passo, dentre vários outros algorítmos:

* [Métodos de Newton][newton]
* Métodos de Quasi-Newton
* Adam

Esses algoritmos usam a idéia de [momento][momento_linear] na hora de dar nossos passos. Outros ainda usam também a segunda derivada para calcular cada passo. Não vamos explicá-los nesse post.
...
Explicamos o que são os parâmetros e os hyperparâmetros. Esse parâmetros também são chamados de:
